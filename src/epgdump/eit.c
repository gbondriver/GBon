...
static int parseEIThead(unsigned char *data, EIThead *h) {
    int boff = 0;
...
static int parseEITbody(unsigned char *data, EITbody *b)
{
    int boff = 0;
    int tnum;
    char buf[4];
...
/* 短形式イベント記述子 */
static int parseSEVTdesc(unsigned char *data, SEVTdesc *desc) {
    int boff = 0;
...
static int parseComponentDesc(unsigned char *data, ComponentDesc *desc) {
    int boff = 0;
...
static int parseAudioComponentDesc(unsigned char *data,AudioComponentDesc *desc) {
    int boff = 0;
...
static int parseContentDesc(unsigned char *data, ContentDesc *desc) {
    int boff = 0,i,j;
...
static int parseSeriesDesc(unsigned char *data, SeriesDesc *desc) {
    int boff = 0;
...
/* 拡張形式イベント記述子 */
static int parseEEVTDhead(unsigned char *data, EEVTDhead *desc) {
    int boff = 0;
...
static int parseEEVTDitem(unsigned char *data, EEVTDitem *desc,EIT_CONTROL *eitcur) {
    int boff = 0;
...
static int parseEEVTDtail(unsigned char *data, EEVTDtail *desc) {
    int boff = 0;
...
static int checkEEVTDitem(EEVTDitem *save, EEVTDitem *new, int descriptor_number) {
...
static EIT_CONTROL	*searcheit(EIT_CONTROL *top, int servid, int eventid)
{
    EIT_CONTROL	*cur ;
    cur = top ;
...
static char	*strstr_eucjp(const char *str, const char *search)
{
    char *pos ;
    pos = (char *)str ;
...
static void	conv_title_subtitle(EIT_CONTROL *eitptr)
{
    int		lp = 0 ;
    //	size_t	addsize ;
    char	*ptr ;
